...
async function handleLiveSegment(meetingId, detail) {
  ...
  await window.api.storage.addTranscriptSegments(meetingId, [{
    speakerId,
...
async function advanceSpeakerSlot(meetingId) {
  if (speakerRoundRobin.length < MAX_AUTO_SPEAKERS) {
    ...
    const speaker = await window.api.storage.upsertSpeaker(meetingId, {
...
async function reconcileEditorText() {
  ...
  for (const node of qsa('.segment-text', editorEl)) {
    ...
    if (segment && segment.text !== newText) {
      await window.api.storage.updateTranscriptSegment(currentMeetingId, segmentId, { text: newText });
  ...
  for (const segment of editorState.segments) {
    if (!seen.has(segment.id) && segment.text !== '') {
      await window.api.storage.updateTranscriptSegment(currentMeetingId, segment.id, { text: '' });
...
async function persistSegmentText(segmentId, text) {
  await window.api.storage.updateTranscriptSegment(currentMeetingId, segmentId, { text });
...
async function toggleHighlight() {
  ...
  await runAction({
    ...
    do: async () => {
      for (const id of ids) {
        await window.api.storage.updateTranscriptSegment(currentMeetingId, id, { highlighted: newState ? 1 : 0 });
    ...
    undo: async () => {
      for (const { id, was } of previous) {
        await window.api.storage.updateTranscriptSegment(currentMeetingId, id, { highlighted: was ? 1 : 0 });
...
function startTimestampEdit(segment, timestampNode) {
  ...
  const commit = async () => {
    ...
    await runAction({
      ...
      do: async () => {
        await window.api.storage.updateTranscriptSegment(currentMeetingId, segment.id, { start_ms: parsedMs });
        setSegmentCacheField(segment.id, 'start_ms', parsedMs);
      },
      undo: async () => {
        await window.api.storage.updateTranscriptSegment(currentMeetingId, segment.id, { start_ms: oldStart });
...
function openSpeakerMenu(segment) {
  ...
  const addNewButton = el('button', { class: 'btn btn-block btn-primary', type: 'button', onClick: async () => {
    ...
    const created = await window.api.storage.upsertSpeaker(meeting.id, {
...
async function reassignSpeaker(segment, newSpeakerId) {
  ...
  await runAction({
    ...
    do: async () => {
      await window.api.storage.updateTranscriptSegment(currentMeetingId, segment.id, { speaker_id: newSpeakerId });
      setSegmentCacheField(segment.id, 'speaker_id', newSpeakerId);
    },
    undo: async () => {
      await window.api.storage.updateTranscriptSegment(currentMeetingId, segment.id, { speaker_id: oldSpeakerId });
      setSegmentCacheField(segment.id, 'speaker_id', oldSpeakerId);
    },
  });
  const refreshed = await window.api.storage.getMeeting(currentMeetingId);
  store.set('currentMeeting', refreshed);
...
async function addBookmarkFromEditorSelection() {
  ...
  await window.api.storage.addBookmark(currentMeetingId, { timeMs: segment.start_ms, label: '' });
  const refreshed = await window.api.storage.getMeeting(currentMeetingId);
  store.set('currentMeeting', refreshed);
  ...
  const body = el('div', {}, [
    ...
    el('div', { class: 'modal-actions' }, [
      ...
      el('button', { class: 'btn btn-primary', type: 'button', onClick: async () => {
        ...
        if (!text) return;
        await window.api.storage.addComment(currentMeetingId, { segmentId: segment.id, text });
        const refreshed = await window.api.storage.getMeeting(currentMeetingId);
        store.set('currentMeeting', refreshed);
        close();
      } }, 'Add comment'),
    ...
    const card = el('div', { class: `comment-card${comment.resolved ? ' resolved' : ''}` }, [
      ...
      el('div', { class: 'modal-actions' }, [
        el('button', { class: 'btn btn-small', type: 'button', onClick: async () => {
          await window.api.storage.resolveComment(currentMeetingId, comment.id, !comment.resolved);
          const refreshed = await window.api.storage.getMeeting(currentMeetingId);
          store.set('currentMeeting', refreshed);
        } }, comment.resolved ? 'Reopen' : 'Resolve'),
        el('button', { class: 'btn btn-small btn-danger', type: 'button', onClick: async () => {
          await window.api.storage.deleteComment(currentMeetingId, comment.id);
          const refreshed = await window.api.storage.getMeeting(currentMeetingId);
          store.set('currentMeeting', refreshed);
        } }, 'Delete'),
      ]),
...
async function openVersionHistory() {
  const versions = await window.api.storage.listTranscriptVersions(currentMeetingId);
  ...
  const listNodes = versions.length
    ? versions.map((version) => el('div', { class: 'backup-row' }, [
      ...
      el('button', { class: 'btn btn-small', type: 'button', onClick: async () => {
        await window.api.storage.restoreTranscriptVersion(currentMeetingId, version.id);
        const refreshed = await window.api.storage.getMeeting(currentMeetingId);
        store.set('currentMeeting', refreshed);
...
function restartAutosaveTimer(settings) {
  ...
  autosaveIntervalHandle = setInterval(async () => {
    if (dirtySinceLastSnapshot && currentMeetingId) {
      await window.api.storage.saveTranscriptSnapshot(currentMeetingId, 'Autosave');
