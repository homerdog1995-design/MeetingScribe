...
const persistTitle = debounce(async (meetingId, value) => {
  const updated = await storage.updateMeeting(meetingId, { title: value.trim() || 'Untitled meeting' });
...
function wireMeetingHeaderActions() {
  ...
  qs('#btn-archive-meeting').addEventListener('click', async () => {
    const meeting = store.get('currentMeeting');
    if (!meeting) return;
    const updated = await storage.archiveMeeting(meeting.id, !meeting.archived);
...
function buildTagEditor(meeting) {
  ...
  meeting.tags.forEach((tag) => {
    ...
    removeButton.addEventListener('click', async () => {
      ...
      await storage.setTags(meeting.id, remaining);
  ...
  input.addEventListener('keydown', async (event) => {
    ...
    await storage.setTags(meeting.id, nextTags);
    const refreshed = await storage.getMeeting(meeting.id);
    store.set('currentMeeting', refreshed);
  });
