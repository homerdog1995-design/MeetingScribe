...
export function initLibrary() {
  ...
  window.api.shortcuts.onNewMeeting(() => createNewMeeting());
...
async function createNewMeeting() {
  ...
  const meeting = await window.api.storage.createMeeting({ quality: settings?.recordingQuality || 'standard' });
  store.set('currentMeeting', meeting);
  store.set('currentView', 'meeting');
}

export async function openMeetingById(id) {
  const meeting = await window.api.storage.getMeeting(id);
...
export async function refreshLibrary() {
  const { items, total } = await window.api.storage.listMeetings({
...
function buildMeetingCard(meeting) {
  ...
  btnDuplicate.addEventListener('click', async (event) => {
    ...
    await window.api.storage.duplicateMeeting(meeting.id);
  ...
  btnArchive.addEventListener('click', async (event) => {
    event.stopPropagation();
    await window.api.storage.archiveMeeting(meeting.id, !meeting.archived);
    refreshLibrary();
  });
...
async function refreshTagSidebar() {
  const tags = await window.api.storage.listAllTags();
